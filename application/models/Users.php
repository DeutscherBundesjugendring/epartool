...
//             $this->_flashMessenger->addMessage('Die angegebene E-Mail-Adresse existiert schon!', 'error');
    ...
    public function recoverPassword($email)
    {
        ...
        if ($validator->isValid($email)) {
            if ($this->emailExists($email)) {
                $newPassword = $this->getRandString();
     ...
     * @param  integer $length  The length of the string. Defaults to value specified in config.
     * @return string           The random string
     */
    protected function getRandString($length = null)
    {
        $passConf = Zend_Registry::get('systemconfig')->security->password;
        if ($length === null) {
            $length = $passConf->length;
        }

        $randString="";
        $possible = $passConf->allowedChars;
        ...
        // set up a counter for how many characters are in the randString so far
        $i = 0;

        // add random characters to $randString until $length is reached
        while ($i < $length) {
            // pick a random character from the possible ones
            $char = substr($possible, mt_rand(0, $maxlength-1), 1);

            // have we already used this character in $randString?
            if (!strstr($randString, $char)) {
